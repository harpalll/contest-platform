...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success) {
        let userPayload = res.data.data.user;
        const tokenPayload = res.data.data.token;

        // Fallback for un-restarted backend server
        if (!userPayload) {
          try {
        ...
        if (userPayload) {
          ...
          setError('Failed to retrieve user data from server.');
        }
      } else {
